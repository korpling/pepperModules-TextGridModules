...
import org.corpus_tools.pepperModules.textgrid.TextGridExporter;
 ...
 * {@link TextGridExporter} class. Feel free to adapt and enhance this test class
 ...
 */
public class TextGridExporterTest extends PepperExporterTest {
	/**
	...
	@Before
	public void setUp() {
		setFixture(new TextGridExporter());
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("TextGridExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("TextGridExporter").getAbsolutePath() + "/rootCorpus");
